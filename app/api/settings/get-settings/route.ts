...
export async function GET() {
    ...
    try {
        ...
        // 2. Fetch settings for the authenticated user, including slots 1-6 AND summary_model
        const selectQuery = `
            slot_1_model,
            slot_2_model,
            slot_3_model,
        ...
        if (fetchError) {
            ...
            if (fetchError.message.includes('column') && fetchError.message.includes('does not exist')) {
                 console.error("!!! DB Schema Mismatch: Column likely missing in 'user_settings'. Ensure 'summary_model' column exists. !!!");
