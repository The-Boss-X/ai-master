...
interface SaveSettingsPayload {
    ...
    summary_model?: string | null; // ADDED: Summary model selection
    ...
    anthropic_api_key?: string | null; // Plain text Anthropic key received from client
}
...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const settingsDataToSave: Record<string, any> = {
            ...
            summary_model: settingsPayload.summary_model || null, // ADDED: Save summary model
        };
        ...
        if (encryptedAnthropicKey) settingsDataToSave.anthropic_api_key_encrypted = encryptedAnthropicKey;

        // 5. Use upsert to insert or update the user's settings row
        // Select back model slots AND summary model to return to client
        const selectColumns = [
            'slot_1_model', 'slot_2_model', 'slot_3_model',
            'slot_4_model', 'slot_5_model', 'slot_6_model',
            'summary_model' // ADDED: Select summary model back
        ].join(', ');

        const { data, error: upsertError } = await supabase
            .from('user_settings')
            .upsert(settingsDataToSave, { onConflict: 'user_id' })
            .select(selectColumns) // Select model fields back
            .single();
